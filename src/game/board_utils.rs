...
pub fn is_symetrical(pattern: u64, len: usize) -> bool {
    let mut i = 0;
    while i < len / 2 {
...
pub fn make_inverted_table() -> HashMap<String, HashMap<u64, (usize, i32)>> {
    let pattern_table = make_pattern_table();
    let mut inverted = HashMap::new();
    for (name, map) in &pattern_table {
        let mut imap = HashMap::new();
        for (pattern, (len, score)) in map {
            imap.insert(invert_pattern(*pattern, *len), (*len, *score));
        }
...
pub fn make_pattern_table() -> HashMap<String, HashMap<u64, (usize, i32)>> {
    let mut hmap = HashMap::new();

    // Make Illegal Three patterns 
    let mut free_three_map = HashMap::new();
    free_three_map.insert(DOUBLE_THREE as u64, (5 as usize, 1 as i32));
    free_three_map.insert(276 as u64, (6 as usize, 1 as i32));
    free_three_map.insert(324 as u64, (6 as usize, 1 as i32));
    hmap.insert("free_threes".into(), free_three_map);

    // Five in a row pattern
    let mut five_map = HashMap::new();
    five_map.insert(341 as u64, (5 as usize, 1 as i32));
    hmap.insert("five_in_a_row".into(), five_map);

    // Pattern Table for scoring 
    let mut score_map = HashMap::new();
    // Five in a row
    score_map.insert(341 as u64, (5 as usize, 100000 as i32));
